...
On POSIX, newswatcher manages exactly one crontab line tagged with a marker comment, so
installing or removing it never disturbs the user's other cron jobs. On Windows it manages
one scheduled task named ``newswatcher-poll``. Interval parsing accepts plain minutes
...
def _run_scheduler_command(
    command: list[str], **kwargs: object) -> subprocess.CompletedProcess[str]:
    ...
    try:
        return subprocess.run(  # type: ignore[call-overload, no-any-return]
            command, timeout=_SUBPROCESS_TIMEOUT_SECONDS, **kwargs)
...
def resolve_poll_command() -> list[str]:
    """The command cron runs each tick: this interpreter's ``python -m newswatcher poll``."""
    return [sys.executable, "-m", "newswatcher", "poll"]


def install_poll(every_minutes: int) -> str:
    """Install (or replace) the recurring newswatcher poll at ``every_minutes``; return the
...
def _schtasks(*args: str, check: bool = True) -> subprocess.CompletedProcess[str]:
    ...
    result = _run_scheduler_command([_schtasks_bin(), *args], capture_output=True,
...
def remove_poll() -> bool:
    """Remove the newswatcher poll job; return whether one was present. crontab on
...
def poll_status() -> str | None:
    """The installed newswatcher poll schedule line, or None when not installed.
...
def _win_schedule_from_xml(xml: str) -> str:
    ...
    return "on a schedule newswatcher did not set"   # hand-edited in Task Scheduler
...
def _read_crontab() -> list[str]:
    result = _run_scheduler_command([_crontab_bin(), "-l"], capture_output=True, text=True)
...
def _write_crontab(lines: list[str]) -> None:
    payload = "\n".join(lines) + "\n" if lines else ""
    result = _run_scheduler_command([_crontab_bin(), "-"], input=payload, text=True,
                                    capture_output=True)
    if result.returncode != 0:
