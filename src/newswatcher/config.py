"""newswatcher's non-secret settings, and the base directories for everything it
...
A setting is read from the environment first (``NEWSWATCHER_DIGEST_TO`` ...), then
...
def config_dir() -> Path:
    ...
    ``$XDG_CONFIG_HOME/newswatcher`` when that variable is set, else
    ``~/.config/newswatcher`` -- the same on every OS (the git / ssh / aws
...
def data_dir() -> Path:
    ...
    A ``data_dir`` in ``config.toml`` (or the ``NEWSWATCHER_DATA_DIR`` env var) wins,
    ...
    ``$XDG_DATA_HOME/newswatcher``, else ``~/.local/share/newswatcher`` (the same on every
    ...
    """
    override = _dir_override("NEWSWATCHER_DATA_DIR")
...
def state_dir() -> Path:
    ...
    A ``state_dir`` in ``config.toml`` (or ``NEWSWATCHER_STATE_DIR``) wins, as an
    ...
    not. Otherwise ``$XDG_STATE_HOME/newswatcher``, else ``~/.local/state/newswatcher``
    ...
    """
    override = _dir_override("NEWSWATCHER_STATE_DIR")
    if override is not None:
        return override
...
def setting(env_name: str) -> str | None:
    ...
    ``env_name`` is the environment-variable spelling (``NEWSWATCHER_DIGEST_TO``); in
    the file it is the same key without the ``NEWSWATCHER_`` prefix, lower-cased
    ...
    if from_env:
        return from_env
    value = load_settings().get(env_name.removeprefix("NEWSWATCHER_").lower())
...
def _xdg_app_dir(env_name: str, home_subpath: str) -> Path:
    """newswatcher's directory under one XDG base: ``$<env>/newswatcher`` when the env
    var holds an absolute path, else ``~/<home_subpath>/newswatcher`` (the XDG spec's
