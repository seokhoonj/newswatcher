...
def test_disallow_blocks_only_matching_paths():
    gate = RobotsGate("newswatcher", _robots(
        "User-agent: *\nDisallow: /admin/\n"))
...
def test_missing_robots_allows_all():
    gate = RobotsGate("newswatcher", _robots(None))  # no robots.txt -> allow
...
def test_throttle_waits_between_fetches_to_a_host():
    ...
    slept: list[float] = []
    gate = RobotsGate("newswatcher", _robots("User-agent: *\nCrawl-delay: 2\n"),
...
def test_per_host_cache_fetches_once():
    ...
    def fetch(url):
        calls.append(url)
        return "User-agent: *\nDisallow:\n"
    gate = RobotsGate("newswatcher", fetch)
